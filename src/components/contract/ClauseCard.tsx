...
const riskStyles: Record<RiskLevel, { badge: string; icon: typeof AlertTriangle; label: string; border: string }> = {
  high: {
    badge: "bg-risk-high/10 text-risk-high border-transparent hover:bg-risk-high/20",
    icon: AlertTriangle,
    label: "高风险",
    border: "border-l-risk-high",
  },
  medium: {
    badge: "bg-risk-medium/10 text-risk-medium border-transparent hover:bg-risk-medium/20",
    icon: AlertCircle,
    label: "中风险",
    border: "border-l-risk-medium",
  },
  low: {
    badge: "bg-risk-low/10 text-risk-low border-transparent hover:bg-risk-low/20",
    icon: CheckCircle2,
    label: "低风险",
...
const ClauseCard = ({ clause }: { clause: ContractClause }) => {
  ...
  return (
    <Card
      className={`bg-card overflow-hidden border-l-4 ${style.border} transition-all`}
    >
      <button
        ...
        <div className="px-4 pb-4 space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div className="rounded-lg bg-destructive/5 p-4 space-y-2">
              <p className="text-xs font-semibold text-destructive uppercase tracking-wider">原始条款</p>
            ...
            <div className="rounded-lg bg-success/10 p-4 space-y-2">
              <p className="text-xs font-semibold text-success uppercase tracking-wider">建议修改</p>
              <p className="text-sm text-foreground leading-relaxed">{clause.suggestedText}</p>
            </div>
          </div>

          <div className="rounded-lg bg-muted p-4 space-y-2">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">修改原因</p>
